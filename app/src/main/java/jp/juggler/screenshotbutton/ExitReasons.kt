...
class ExitReasons {
    companion object {
        val log = LogCategory("ExitReasons")

        private fun reasonString(v: Int) = when (v) {
        ...
        fun getExitReasons(context: Context, maxNum: Int = 10): List<String> {
            if (Build.VERSION.SDK_INT < 30) {
                ...
                val am = ContextCompat.getSystemService(context,ActivityManager::class.java)
                    ?: error("can't find ActivityManager")
